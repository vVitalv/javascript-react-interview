## Вопросы на собеседовании frontend JavaScript React.js разработчика в 2022 году
...
4. Использовать метод какой-либо библиотеки, например `_.cloneDeep()` из JavaScript-библиотеки `lodash`
5. Cделать это средствами встроенного объекта JSON:
```javascript
...
1. **Глобальная область видимости** — переменные и функции, объявленные в *глобальном контексте выполнения*, имеют глобальную область видимости и доступны из любого места в коде (не актуально для *модульной* структуры кода).
...
<details><summary>Как использовать глобальные переменные в модульной структуре кода</summary>
<br/>
...
Объект `LexicalEnvironment` является внутренним служебным объектом, он скрыт от прямого доступа.
...
3. `p.finally(onFinally)` - в качестве параметра принимает функцию (без аргументов), которая будет выполнена, когда промис будет завершен (вне зависимости успешно или с ошибкой). Метод `finally()` возвращает *промис*.
...
Для обработки ошибок, которые могут возникнуть в процессе вызова асинхронной операции применяется конструкция `try-catch`. Конструкция `try-catch`, в отличие от метода `catch()` объекта `Promise`, позволяет обработывать не только ошибки внутри асинхронных операций, но и вызовы исключений и программные ошибки.
...
**Цикл событий** (*Event Loop*) - это механизм, который отвечает за выполнение кода, сбор и обработку событий и выполнение подзадач из очереди.
...
Свойство `__proto__` объекта `Object.prototype` является свойством доступа, которое расширяет внутренний прототип `[[Prototype]]` объекта.

> Изменение `[[Prototype]]` объекта является очень медленной операцией, поэтому никогда не нужно его изменять. Вместо этого можно создать объект с нужным прототипом `[[Prototype]]`, с помощью метода `Object.create()`.

[Подробнее](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/proto)
